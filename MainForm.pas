...
if WriteFile(fh,data,size,tmp,nil)=true then
result:=true;
CloseHandle(fh);
end;
End;
...
pshdr:=psHeader(buffer);
buffer:=pbyte(cardinal(buffer) + 8);
case pshdr.sType of
$7A:
Begin
...
buffer:=pbyte(cardinal(buffer) + pshdr.sLength);
end;
result:=buffer;
End;
...
if Cardinal(pCardinal(fByte)^)<>0 then begin
tmp:=Cardinal(pCardinal(fbyte)^);//getStrCount
fByte:=TraceStrings(pbyte(cardinal(fbyte) + 4),tmp);
end
else fByte:=pbyte(cardinal(fByte) + 4);
...
fByte:=pbyte(cardinal(fByte) + 4);
for I := 0 to fcount do begin
tmp:=Cardinal(pCardinal(fByte)^);
DecryptSection(pbyte(cardinal(fByte) + 8),tmp);
SaveFile(unpDir+WideString(FileNamesA[I]),pbyte(cardinal(fByte) + 8),tmp,overwrite);
fByte:=pbyte(cardinal(fByte) + 8+tmp);
end;
